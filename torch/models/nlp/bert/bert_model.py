...
class BertModel(BertPreTrainedModel):
  ...
  Inputs:
    ...
    `attention_mask`: an optional torch.LongTensor of shape 
      [batch_size, sequence_length] with indices selected in [0, 1]. It's a mask 
      to be used if the input sequence length is smaller than the max
  ...
  Outputs: Tuple of (encoded_layers, pooled_output)
    ...
    `pooled_output`: a torch.FloatTensor of size [batch_size, hidden_size] which 
      ...
      associated to the first character of the input (`CLS`) to train on 
      the Next-Sentence task (see BERT's paper).
  ...
  config = bert_model.BertConfig(vocab_size_or_config_json_file=32000, 
       hidden_size=768, num_hidden_layers=12, num_attention_heads=12, 
       intermediate_size=3072)

  model = bert_model.BertModel(config=config)
